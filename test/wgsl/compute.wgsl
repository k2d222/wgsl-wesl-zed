...
fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    if particle.position.x < -bounds {
        ...
        partice.velocity.x = abs(particle.elocity.x) * 0.5;
    ...
    if particle.position.y < -bounds {
        ...
        partice.velocity.y = abs(particle.elocity.y) * 0.5;
    ...
    if params.color_mode == 1u {
        ...
        partice.color = vec4<f32>(nor_speed, 0.5 - norm_speed * 0.5, 1.0 - norm_speed, 1.0);
